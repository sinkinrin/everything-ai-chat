  ...
  settings: {
    ...
    systemPrompt: {
      ...
      tips: {
        ...
        guidance: 'Четкое руководство: Скажите ИИ, как понимать намерение поиска и преобразовывать в синтаксис Everything',
        examples: 'Включить примеры: Включение некоторых примеров преобразования в промпт улучшит точность',
        concise: 'Сохранять краткость: Избегайте слишком сложных инструкций, сохраняйте промпт ясным и понятным',
        test: 'Тестировать эффективность: После изменения вы можете проверить эффективность через реальный поиск'
      }
    },
